...
{
    Reader(Tokenizer::iterator& beg, Tokenizer::iterator end) : first_(beg), last_(end) {}
...
bool Session::processRequest(StringPiece request)
{
    ...
    ++beg;
    if (command_ == "set" || command_ == "add" || command_ == "replace" || command_ == "append" || command_ == "prepend"
        || command_ == "cas")
    {
