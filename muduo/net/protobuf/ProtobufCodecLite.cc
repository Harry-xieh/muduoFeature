...
int ProtobufCodecLite::serializeToBuffer(const google::protobuf::Message& message, Buffer* buf)
{
    ...
    /**
     * 'ByteSize()' of message is deprecated in Protocol Buffers v3.4.0 firstly. But, till to
     * v3.11.0, it just getting start to be marked by '__attribute__((deprecated()))'. So, here,
     * v3.9.2 is selected as maximum version using 'ByteSize()' to avoid potential effect for
     * previous muduo code/projects as far as possible. Note: All information above just INFER from
     ...
     * PROTOBUF_DEPRECATED_MSG(msg) __attribute__((deprecated(msg)))'. In addition, usage of
     * 'ToIntSize()' comes from Impl of ByteSize() in new version's Protocol Buffers.
     */
