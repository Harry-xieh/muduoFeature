...
Hiredis::Hiredis(EventLoop* loop, const InetAddress& serverAddr) : loop_(loop), serverAddr_(serverAddr), context_(NULL)
{}
...
void Hiredis::logConnection(bool up) const
{
    ...
    LOG_INFO << localAddr.toIpPort() << " -> " << peerAddr.toIpPort() << " is " << (up ? "UP" : "DOWN");
}
