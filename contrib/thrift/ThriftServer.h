...
class ThriftServer : boost::noncopyable, public TServer
{
public:
    ThriftServer(const boost::shared_ptr<TProcessorFactory>& processorFactory, muduo::net::EventLoop* eventloop,
                 const muduo::net::InetAddress& addr, const muduo::string& name)
        : TServer(processorFactory),
          server_(eventloop, addr, name),
          numWorkerThreads_(0),
          workerThreadPool_(name + muduo::string("WorkerThreadPool"))
    {
        server_.setConnectionCallback(std::bind(&ThriftServer::onConnection, this, muduo::_1));
    }

    ThriftServer(const boost::shared_ptr<TProcessor>& processor, muduo::net::EventLoop* eventloop,
                 const muduo::net::InetAddress& addr, const muduo::string& name)
        : TServer(processor),
          server_(eventloop, addr, name),
          numWorkerThreads_(0),
    ...
    ThriftServer(const boost::shared_ptr<TProcessorFactory>& processorFactory,
                 const boost::shared_ptr<TProtocolFactory>& protocolFactory, muduo::net::EventLoop* eventloop,
                 const muduo::net::InetAddress& addr, const muduo::string& name)
        : TServer(processorFactory),
          server_(eventloop, addr, name),
          numWorkerThreads_(0),
    ...
    ThriftServer(const boost::shared_ptr<TProcessorFactory>& processorFactory,
                 ...
                 const boost::shared_ptr<TProtocolFactory>& outputProtocolFactory, muduo::net::EventLoop* eventloop,
                 const muduo::net::InetAddress& addr, const muduo::string& name)
        : TServer(processorFactory),
          server_(eventloop, addr, name),
          numWorkerThreads_(0),
